...
export class MessageChannelService {
  ...
  async createMessage(dto: SendMessageDto, isSilent: boolean = false) {
    let rM: IRoomMember = await this.middlewareService.isThereRoomMember(
      dto._roomId,
      dto.myUser._id,
      "rT t bId isOneSeen disappearingTimer"
    );
    ...
    if (rM.disappearingTimer && rM.disappearingTimer > 0) {
      const expirationTimeInMs = rM.disappearingTimer * 1000;

      // Add the calculated date to a new property on the DTO
      (dto as any).disappearAt = new Date(Date.now() + expirationTimeInMs);
    }
    ...
    if (isExits)
      throw new ForbiddenException(
        "Message already in database ForbiddenException"
      );
    if (dto.replyToLocalId) {
      let rToMsg: IMessage | null = await this.messageService.getByLocalId(
        dto.replyToLocalId
      );
      if (!rToMsg)
        throw new ForbiddenException(
          "dto.replyToId msg not exist in db " + dto.replyToLocalId
        );
      //todo s3 support
    ...
    if (isSingle || isOrder) {
      ...
      if (!peer)
        throw new BadRequestException(
          "Cant find the peer user in the chat data is"
        );
      ...
      if (!isSilent)
        await this.notificationService.singleChatNotification(
          peer.uId,
          newMessage
        );
      return newMessage;
  ...
  async deleteRoomMessage(dto: DeleteMessageDto) {
    ...
    if (dto.type == DeleteMessageType.me) {
      ...
      let [m] = await Promise.all([
        this.messageService.getByIdOrFail(dto.messageId),
      ]);
      ...
      let x: IMessage = await this.messageService.findByIdAndUpdate(
        dto.messageId,
        {
          dltAt: new Date(),
        }
  ...
  async getRoomMessages(myId: string, roomId: string, dto: MessagesSearchDto) {
    ...
    let res = await this.messageService.findAllMessagesAggregation(
      newMongoObjId(myId),
  ...
  private async getMessageAttachment(dto: SendMessageDto) {
    if (dto.isInfo()) {
      if (!dto._messageAttachment)
        throw new BadRequestException(
          "for isInfo message we must include _messageAttachment"
        );
    ...
    if (dto.isVoice()) {
      ...
      if (!dCodedAtt["duration"])
        throw new BadRequestException(
          "Voice duration in milli second is required in attachment json"
        );
    ...
    if (dto.isVideo()) {
      ...
      if (dto._secondMediaFile) {
        ...
        thumbImageData["blurHash"] =
          dCodedAtt["thumbImage"]["blurHash"] ?? null;
        thumbImageData["height"] = imgData.height;
        thumbImageData["name"] = dto._secondMediaFile.originalname;
        thumbImageData["fileHash"] = this.sha256FromBuffer(
          dto._secondMediaFile.buffer
        );
      }
    ...
    if (dto.isLocation()) {
      ...
      if (!att["lat"])
        throw new BadRequestException("lat is required as String");
      if (!att["long"])
        throw new BadRequestException("long is required as String");
      if (!att["linkPreviewData"])
        throw new BadRequestException("linkPreviewData is required as {}");
      if (!att["linkPreviewData"]["title"])
        throw new BadRequestException(
          "linkPreviewData title is required as string"
        );
      if (!att["linkPreviewData"]["description"])
        throw new BadRequestException(
          "linkPreviewData description is required as string"
        );
  ...
  private async getForwardMessageNewDto(dto: SendMessageDto) {
    ...
    let fToMsg: any = await this.messageService.getByLocalId(
      dto.forwardLocalId
    );
    if (!fToMsg)
      throw new ForbiddenException(
        "cant find the forwarded message id " + dto.forwardLocalId
      );
  ...
  private async _createStatusForGroupChat(_roomId: string, mId: string) {
    ...
    await this.groupMessageStatusService.createMany(
      members.map((value) => {
        return {
          mId: mId,
          rId: _roomId,
  ...
  async starRoomMessage(dto: RoomIdAndMsgIdDto) {
    ...
    if (msg.rId.toString() != dto.roomId)
      throw new BadRequestException("message not exists in this room!");
  ...
  async oneSeeThisMessage(dto: RoomIdAndMsgIdDto) {
    ...
    if (!msg.isOneSeen)
      throw new BadRequestException(
        "Msg cant update is should be one seen true"
      );
  ...
  async reactToMessage(dto: RoomIdAndMsgIdDto, emoji: string) {
    ...
    const hadThisReaction =
      reactions[emoji] && (reactions[emoji] as any).userIds?.includes(userId);

    // Remove user's existing reaction if any (from any emoji)
    for (const [existingEmoji, reactionData] of Object.entries(reactions)) {
      if (
        reactionData &&
        typeof reactionData === "object" &&
        "userIds" in reactionData
      ) {
