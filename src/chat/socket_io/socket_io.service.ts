...
export class SocketIoService {
  ...
  async handleConnection(client: Socket, ...args: any[]) {
    this.userSocketMap.set(client.user._id.toString(), client.id);
    ...
    for (let r of roomsIds) {
      if (r.rT == RoomType.Single || r.rT == RoomType.Order) {
        ...
        if (isUpdated) {
          this.io.to(r.rId.toString()).emit(
  ...
  async myOnline(decodedList: any[]) {
    ...
    let onlineSockets = await this.getOnlineFromList(
      decodedList.map((data) => data["peerId"])
    );
    let res = [];
  ...
  async handleDisconnect(client: Socket) {
    this.userSocketMap.delete(client.user._id.toString());
    await this._setLastSeenAt(client.user);
    let rId = client["typingTo"];
    if (rId) {
      this.io.to(rId.toString()).emit(
    ...
    await this._checkIfThisDeviceInCall(
      client.user._id,
      client.user.currentDevice._id
    );
  }
  ...
  private async _checkIfThisDeviceInCall(userId: string, deviceId: string) {
    ...
    if (
      ...
      let theUser = await this.userService.findById(
        userId,
