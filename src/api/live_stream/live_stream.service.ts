...
import {
  ILiveStream,
  ILiveStreamParticipant,
  ILiveStreamMessage,
  ILiveStreamJoinRequest,
  LiveStreamStatus,
  ParticipantRole,
  LiveStreamType,
...
export class LiveStreamService {
  constructor(
    ...
    private readonly liveStreamModel: Model<ILiveStream>,
    @InjectModel("LiveStreamParticipant")
    private readonly participantModel: Model<ILiveStreamParticipant>,
    @InjectModel("LiveStreamMessage")
    private readonly messageModel: Model<ILiveStreamMessage>,
    @InjectModel("LiveStreamJoinRequest")
    private readonly joinRequestModel: Model<ILiveStreamJoinRequest>,
  ...
  async saveLiveStreamRecording(
    ...
    // 1. Verification: Ensure the stream exists and the uploader is the original streamer.
    if (!stream) {
      throw new NotFoundException("Live stream not found");
    }
    if (stream.streamerId !== user._id.toString()) {
      throw new ForbiddenException(
        "You are not authorized to upload a recording for this stream."
      );
    }
    if (stream.status !== LiveStreamStatus.ENDED) {
      // Optional: Ensure the stream has officially ended before accepting a recording.
      throw new BadRequestException(
        "Stream must be in ENDED status to upload a recording."
      );
    }

    // 2. Upload the file using your uploader service
    // We will create the 'uploadLiveStreamVideo' method in the next step.
    const recordingKey = await this.fileUploaderService.uploadLiveStreamVideo(
      file.buffer,
      stream.streamerId
    );
  ...
  async createLiveStream(dto: CreateLiveStreamDto): Promise<ILiveStream> {
    if (dto.streamType === LiveStreamType.PAID) {
      if (!dto.price || dto.price <= 0 || !dto.currency) {
        throw new BadRequestException(
    ...
    const liveStream = await this.liveStreamModel.create({
      ...
      isPrivate:
        dto.streamType === LiveStreamType.PAID ? true : dto.isPrivate || false,
      requiresApproval: dto.requiresApproval || false,
      allowedViewers: dto.allowedViewers || [],
      tags: dto.tags || [],
      thumbnailUrl: dto.thumbnailUrl,
      savePreference: dto.savePreference,
      savedViewersInclude:
        dto.savePreference === LiveStreamSavePreference.SAVE_SPECIFIC_INCLUDE
          ? dto.savedViewersInclude
          : [],
      savedViewersExclude:
        dto.savePreference === LiveStreamSavePreference.SAVE_SPECIFIC_EXCLUDE
          ? dto.savedViewersExclude
          : [],
  ...
  async endLiveStream(streamId: string, userId: string): Promise<ILiveStream> {
    ...
    if (stream.savePreference !== LiveStreamSavePreference.DO_NOT_SAVE) {
      // This process should be handled by a background job so it doesn't slow down the API response.
      // Here, you would trigger your new RecordingService or add a job to a queue.
      // this.recordingService.startProcessing(stream);
      console.log(`Triggering recording process for stream: ${streamId}`);
    }
    await stream.save();
  ...
  async joinLiveStream(
    dto: JoinLiveStreamDto
  ): Promise<{ stream: ILiveStream; agoraToken: string }> {
    ...
    if (
      ...
      if (!approvedRequest) {
        throw new ForbiddenException(
          "You need approval from the host to join this stream. Please request to join first."
        );
      }
    }
  ...
  async sendMessage(
    streamId: string,
    dto: SendLiveStreamMessageDto
  ): Promise<ILiveStreamMessage> {
    const stream = await this.liveStreamModel.findById(streamId);
    if (!stream) {
      throw new NotFoundException("Live stream not found");
    }
    ...
    if (dto.messageType === "gift" && dto.giftData && dto.giftData.giftPrice) {
      try {
        ...
        this.socketService.io.to(stream.streamerId).emit("gift_auto_claimed", {
          ...
          message: `Received a gift worth $${dto.giftData.giftPrice.toFixed(
            2
          )} from ${dto.myUser.fullName}!`,
        });
      } catch (error) {
  ...
  async removeParticipant(
    ...
    if (participant.userId === stream.streamerId) {
      throw new BadRequestException(
        "Cannot remove the streamer from their own stream"
      );
    }
  ...
  async banParticipant(
    ...
    if (participant.userId === stream.streamerId) {
      throw new BadRequestException(
        "Cannot ban the streamer from their own stream"
      );
    }
  ...
  async getStreamLikes(
    streamId: string
  ): Promise<{ likesCount: number; likedBy: string[] }> {
  ...
  async requestJoinStream(
    ...
    if (stream.streamerId === dto.myUser._id) {
      throw new BadRequestException(
        "You cannot request to join your own stream"
      );
    }
    ...
    if (stream.isPrivate) {
      throw new BadRequestException(
        "This is a private stream. You need to be invited."
      );
    }

    if (!stream.requiresApproval) {
      throw new BadRequestException(
        "This stream does not require approval. You can join directly."
      );
    }
    ...
    if (existingRequest) {
      if (existingRequest.status === "pending") {
        throw new BadRequestException(
          "You already have a pending join request for this stream"
        );
      } else {
        throw new BadRequestException(
          "You already have approval to join this stream"
        );
      }
    }
  ...
  async respondToJoinRequest(
    ...
    if (stream.streamerId !== dto.myUser._id) {
      throw new ForbiddenException(
        "Only the streamer can respond to join requests"
      );
    }
    ...
    if (dto.action === "approve") {
      this.socketService.io
        .to(joinRequest.streamId)
        .emit("join_request_approved", {
  ...
  async getJoinRequests(
    streamId: string,
    userId: string
  ): Promise<ILiveStreamJoinRequest[]> {
    const stream = await this.liveStreamModel.findById(streamId);
    if (!stream) {
      throw new NotFoundException("Live stream not found");
    }
    ...
    return await this.joinRequestModel
      .find({
        streamId: streamId,
        status: "pending",
      })
  ...
  private async sendLiveStreamNotification(stream: ILiveStream): Promise<void> {
    try {
      ...
      for (const user of allUsers) {
        const userTokens = await this.userDeviceService.getUserPushTokens(
          user._id.toString()
        );
        console.log(`User ${user._id} tokens:`, {
          fcm: userTokens.fcm.length,
          oneSignal: userTokens.oneSignal.length,
        });
      ...
      console.log(
        `Total tokens collected - FCM: ${tokens.fcm.length}, OneSignal: ${tokens.oneSignal.length}`
      );
    } catch (error) {
