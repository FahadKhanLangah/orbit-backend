...
import { Model,Query, FilterQuery, QueryOptions, UpdateQuery } from "mongoose";
import { BaseService } from "../../core/common/base.service";
import { IGift } from "./entities/gift.entity";

@Injectable()
export class GiftService extends BaseService<IGift> {
  constructor(@InjectModel("gift") private readonly model: Model<IGift>) {
    super();
  }
  findByIdGift(
    id: string,
    select?: string | null | undefined
  ): Query<IGift | null, IGift> {
    // Return the query directly. REMOVED Promise.resolve()
  ...
  findByIdAndUpdate(id: string, update: Partial<IGift>): Promise<IGift | null> {
  ...
  updateMany(
    filter: FilterQuery<IGift>,
    update: UpdateQuery<IGift>,
    session?,
    options?: QueryOptions<IGift> | null
  ): Promise<any> {
    return Promise.resolve(
      this.model.updateMany(filter, update, options).session(session)
    );
  }
